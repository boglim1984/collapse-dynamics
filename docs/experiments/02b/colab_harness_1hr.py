...
class HighFidelityTracker:
    ...
    def _register_hooks(self):
        ...
        if self.arch_type == "resnet":
            ...
            # Sort ResNet: layer idx then block idx
            temp_order.sort(key=lambda k: int(k.split(".")[0][-1])*100 + int(k.split(".")[1]))
            ...
            for name, module in self.model.named_modules():
                ...
                if len(parts) == 2 and "blocks" in parts[0] and parts[1].isdigit():
                    ...
                    temp_order.append(name)
            # Sort ViT: block idx
            temp_order.sort(key=lambda k: int(k.split(".")[1]))
        else:
            raise ValueError("unknown arch_type")
        
        # FREEZE order once
        self.layer_order = temp_order
    ...
    def forward(self, x):
        self.clear()
        _ = self.model(x)
        assert all(name in self.activations for name in self.layer_order), "Missing activations for frozen layer order"
    ...
    def train_probes(self, loader, device, probe_epochs: int, lr=1e-3):
        ...
        for _ in range(probe_epochs):
            for x, _y in loader:
                ...
                # FIXED: Enable gradients for probe updates, but keep model forward no-grad.
                with torch.no_grad():
                ...
                with torch.enable_grad():
                    total = 0.0
                    for name in self.layer_order:
                        # detached rep prevents backprop into model even if grads are enabled
                        r = self.get_rep(self.activations[name]).detach()
...
def collect_trajectories(tracker: HighFidelityTracker, loader, device) -> np.ndarray:
    # FIXED: L is frozen based on tracker identity
    L = len(tracker.layer_order)
    ...
    for i, (x, _y) in enumerate(tqdm(loader, desc=f"traj {tracker.arch_type}", leave=False)):
        ...
        with torch.cuda.amp.autocast(enabled=(device.type == "cuda"), dtype=torch.float16):
            ...
            assert margins.shape[1] == L, f"L mismatch: expected {L} but got {margins.shape[1]}"
...
def sweep_alphas(res_traj, vit_traj, alphas, eps_final, req_consecutive):
    ...
    for alpha in alphas:
        td_r = []; td_v = []
        tabs_r = np.empty(N, dtype=np.float32); tabs_v = np.empty(N, dtype=np.float32)
        viol_r = np.empty(N, dtype=np.int32); viol_v = np.empty(N, dtype=np.int32)
        ...
        reach_r = len(td_r) / N; reach_v = len(td_v) / N
        common = min(reach_r, reach_v)
        rows.append(dict(
            alpha=float(alpha), reach_resnet=float(reach_r), reach_vit=float(reach_v),
            common_reach=float(common), reach_gap=float(reach_r - reach_v),
            ...
            mean_viol_resnet=float(np.mean(viol_r)), mean_viol_vit=float(np.mean(viol_v)),
...
def pick_alpha_balanced(df, target_common=0.60, max_gap=0.06):
    ...
    if cand.empty: cand = df[df["common_reach"] >= min(0.50, target_common)].copy()
    ...
    # Selection priority: common_reach -> reach_gap -> tabs_gap -> effect_size
...
# -------------------------
# Falsifiers (Scalar outcomes for cross-arch depth compatibility)
# -------------------------
def null_depth_permute(traj: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    N, L = traj.shape
    out = traj.copy()
    for i in range(N): out[i] = out[i, rng.permutation(L)]
    return out
...
def null_pooled_from_tabs(tabs_r: np.ndarray, tabs_v: np.ndarray, rng: np.random.Generator):
    ...
    N = len(tabs_r)
    r = pool[perm[:N]]; v = pool[perm[N:2*N]]
...
def run_one(run_id: int, seed: int, batch_probe: int, batch_traj: int):
    ...
    probe_loader, traj_loader = make_loaders(DATASET, DATA_ROOT, PROBE_N, TRAJ_N, batch_probe, batch_traj, seed)
    ...
    res_traj = collect_trajectories(tr_r, traj_loader, device) 

    del tr_r, resnet; cleanup_cuda()
    ...
    vit_traj = collect_trajectories(tr_v, traj_loader, device)

    del tr_v, vit, probe_loader, traj_loader; cleanup_cuda()
    ...
    N = res_traj.shape[0]
    tabs_r = np.empty(N, dtype=np.float64); tabs_v = np.empty(N, dtype=np.float64)
    viol_r = np.empty(N, dtype=np.int32); viol_v = np.empty(N, dtype=np.int32)
    td_r = np.full(N, np.nan, dtype=np.float64); td_v = np.full(N, np.nan, dtype=np.float64)
    ...
    pd.DataFrame({"t_abs_resnet": tabs_r, "t_abs_vit": tabs_v, "violations_resnet": viol_r, "violations_vit": viol_v}).to_csv(abs_path, index=False)

    reach_r = float(np.isfinite(td_r).mean()); reach_v = float(np.isfinite(td_v).mean()); common = min(reach_r, reach_v)
    gap_tabs, d_tabs = float(np.mean(tabs_r) - np.mean(tabs_v)), float(cohens_d(tabs_r, tabs_v))
    ci_tabs = bootstrap_ci_gap(tabs_r, tabs_v, n_boot=BOOT_N, seed=seed)
    gap_viol = float(np.mean(viol_r) - np.mean(viol_v))
    ci_viol = bootstrap_ci_gap(viol_r.astype(np.float64), viol_v.astype(np.float64), n_boot=BOOT_N, seed=seed+99)

    nulls = {}
    if DO_NULLS:
        rng = np.random.default_rng(seed + 888)
        if NULL_DEPTHPERM:
            g, d = eval_tabs_gap_d(null_depth_permute(res_traj, rng), null_depth_permute(vit_traj, rng), alpha, EPS_FINAL)
            nulls["depthperm_tabs_gap"], nulls["depthperm_tabs_d"] = g, d
        if NULL_LABELSWAP:
            g, d = null_labelswap_from_tabs(tabs_r, tabs_v, rng)
            nulls["labelswap_tabs_gap"], nulls["labelswap_tabs_d"] = g, d
        if NULL_POOLEDRESAMPLE:
            g, d = null_pooled_from_tabs(tabs_r, tabs_v, rng)
            nulls["pooled_tabs_gap"], nulls["pooled_tabs_d"] = g, d

    # REPRODUCIBILITY SPEC (Consistent with 02B conventions)
    spec = {
        "run_id": int(run_id), "seed": int(seed), "alpha": float(alpha),
        "alpha_grid": [float(ALPHA_LO), float(ALPHA_HI), int(ALPHA_STEPS)],
        "balanced_reach": {"target_common_reach": float(TARGET_COMMON_REACH), "max_reach_gap": float(MAX_REACH_GAP)},
        "eps_final_margin": float(EPS_FINAL), "req_consecutive": int(REQ_CONSEC),
        "probe_n": int(PROBE_N), "traj_n": int(TRAJ_N), "probe_epochs": int(PROBE_EPOCHS),
        "summary": {
            "reach_resnet": reach_r, "reach_vit": reach_v, "common_reach": common,
            "t_abs_gap_R_minus_V": gap_tabs, "t_abs_d_R_minus_V": d_tabs,
            "t_abs_gap_ci95": [float(ci_tabs[0]), float(ci_tabs[1])],
            "viol_gap_R_minus_V": gap_viol, "viol_gap_ci95": [float(ci_viol[0]), float(ci_viol[1])],
        },
        "nulls": nulls,
        "artifacts": {"alpha_sweep_csv": os.path.basename(sweep_path), "absorption_metrics_csv": os.path.basename(abs_path)}
    }
    with open(os.path.join(OUTDIR, f"boundary_spec_run_{run_id:03d}.json"), "w") as f:
        json.dump(spec, f, indent=2)

    return dict(run_id=run_id, seed=seed, alpha=alpha, common_reach=common, t_abs_gap_R_minus_V=gap_tabs, t_abs_d_R_minus_V=d_tabs, **nulls)
...
start = time.time()
rows = []
batch_probe = BATCH_PROBE; batch_traj = BATCH_TRAJ

run_id = 0
while (time.time() - start)/60.0 < RUN_MINUTES and run_id < RUNS_MAX:
    run_id += 1
    seed = SEED_START + (run_id - 1)
    print(f"\n=== Run {run_id} | seed={seed} | elapsed={(time.time()-start)/60.0:.1f} min ===")
    try:
        row = run_one(run_id, seed, batch_probe, batch_traj)
        rows.append(row)
        pd.DataFrame(rows).to_csv(os.path.join(OUTDIR, "runs_summary.csv"), index=False)
        print(f"alpha={row['alpha']:.3f} | gap={row['t_abs_gap_R_minus_V']:.3f} | d={row['t_abs_d_R_minus_V']:.3f}")
    except torch.cuda.OutOfMemoryError:
        print("[OOM] Shrinking batches."); cleanup_cuda()
        batch_probe = max(8, batch_probe//2); batch_traj = max(8, batch_traj//2)
        continue
