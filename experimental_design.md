...
We measure the Softmax probabilities $P(\text{class}_A)$ and $P(\text{class}_B)$ across this path.

## Metric: Collapse Width ($\sigma$)
The **Collapse Width** $\sigma$ is defined as the width of the transition region where the model's confidence is not dominated by either $x_A$ or $x_B$. A smaller $\sigma$ indicates a more brittle and abrupt collapse from one decision to another, suggesting a lack of behavioral robustness.

## Hypotheses
- **H1 (Brittleness)**: Vision Transformers (ViT) will exhibit smaller Collapse Widths ($\sigma$) compared to convolutional architectures (ResNet) at matched accuracy, indicating more brittle decision boundaries.
